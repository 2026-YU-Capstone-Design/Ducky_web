...
function SessionMessage({ message }: { message: ChatMessage }) {
  ...
  return (
    ...
    >
      <div
        className={cn(
          "max-w-[88%] rounded-lg border px-4 py-3 text-sm leading-relaxed shadow-sm break-keep whitespace-pre-wrap",
          isUser
            ? "border-[#FECA43] bg-[#FECA43] text-[#2E2A22]"
            : "border-[#E7DDC8] bg-white text-gray-800",
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden p-5">
      <div className="grid shrink-0 gap-3 sm:grid-cols-3">
        <Card className="rounded-lg border border-[#E7DDC8] bg-[#FAF8F5] py-0">
      ...
      <div className="mt-4 shrink-0 rounded-lg border border-[#E7DDC8] bg-[#FAF8F5] p-4">
      ...
      <div className="mt-5 flex min-h-0 flex-1 flex-col">
        <div className="flex items-center justify-between gap-3">
          <h3 className="text-sm font-bold text-gray-950">대화 내역</h3>
          <span className="shrink-0 text-xs font-medium text-gray-500">
            {session.messages.length}개
          </span>
        </div>
        <ScrollArea className="mt-3 min-h-[14rem] flex-1 rounded-lg border border-[#E7DDC8] bg-[#FAF8F5] sm:min-h-[20rem]">
  ...
  return (
    <section className="mx-auto flex w-full max-w-7xl flex-1 flex-col px-5 py-8 sm:px-8 lg:px-10">
            ...
            <Button
              ...
              className={cn(
                ...
                filter === item.value
                  ...
                  : "border-[#E7DDC8] bg-white hover:bg-[#FFF7E0]",
              )}
            >
              {item.label}
            </Button>
      ...
      {filteredSessions.length === 0 && (
        <div className="mt-5 rounded-lg border border-dashed border-[#E7DDC8] bg-white px-5 py-10 text-center">
          <p className="font-bold text-gray-950">표시할 세션이 없습니다</p>
      ...
      >
        <DialogContent className="flex max-h-[92dvh] min-h-[min(42rem,92dvh)] gap-0 overflow-hidden rounded-lg p-0 sm:max-w-4xl">
          {selectedSession && (
            <>
              <DialogHeader className="shrink-0 border-b border-[#E7DDC8] p-5 pr-12">
                ...
                <DialogTitle className="mt-2 text-xl font-bold leading-snug text-gray-950 break-keep">
                  {selectedSession.title}
                </DialogTitle>
                <DialogDescription className="flex flex-wrap gap-x-4 gap-y-1 text-sm text-gray-500">
                  <span className="inline-flex items-center gap-1.5">
